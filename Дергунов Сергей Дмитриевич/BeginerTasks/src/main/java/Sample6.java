    ...
    * */
    ...
    private static boolean isPrime(int number){
        if (number==0 || number==1 || number==2|| number==3){ //Если число которое нам передали одно из 0 1 2 3
            ...
            for(int i=3;i <= Math.floor(Math.sqrt(number));i+=2){//для цулочисленного i установить значение 2;выполнять блок пока i меньше (number+1/2); после каждого исполнения увеличить i на 1
                count++;
