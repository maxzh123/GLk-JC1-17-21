...
public class Model {
    ...
    public boolean hasEmptyFields(){
        for(int i=0;i< field.length;i++){
            for(int j=0;j< field[i].length;j++){
                if (field[i][j]==Amount.IsEmpty){return true;}
            }
        }
        return false;
    }
    ...
    public Amount getWinner(){
        ...
        for(int i=0;i< CELLS;i++){
            //Проверка строк
            boolean flag=Amount.IsEmpty!=field[i][0];
        ...
        boolean flag=Amount.IsEmpty!=field[0][0];
        ...
        flag=Amount.IsEmpty!=field[0][CELLS-1];
        for(int i=0;flag && i<CELLS;i++){
    ...
    public void doStep(int i, int j, Amount x) throws Exception {
        if (field[i][j]!=Amount.IsEmpty) {throw new Exception("Поищи другое место!");}
        if (x==Amount.IsEmpty || x==null) {throw new Exception("Нельзя такое сюда ставить!");}
